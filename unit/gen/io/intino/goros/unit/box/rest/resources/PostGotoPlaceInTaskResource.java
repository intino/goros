...
public class PostGotoPlaceInTaskResource implements Resource {
	...
	public PostGotoPlaceInTaskResource(UnitBox box, SparkManager manager) {
		this.box = box;
		this.manager = manager;
	}

	public void execute() throws AlexandriaException {
		write(fill(new io.intino.goros.unit.box.actions.PostGotoPlaceInTaskAction()).execute());
	}

	private io.intino.goros.unit.box.actions.PostGotoPlaceInTaskAction fill(io.intino.goros.unit.box.actions.PostGotoPlaceInTaskAction action) {
		action.box = this.box;
		action.context = context();
		action.id = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromQuery("id"), String.class);
		action.place = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromQuery("place"), String.class);
		action.history = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromQuery("history"), String.class);
		return action;
	}
