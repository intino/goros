...
public class PostExportNodesResource implements Resource {
	...
	public PostExportNodesResource(UnitBox box, SparkManager manager) {
		this.box = box;
		this.manager = manager;
	}

	public void execute() throws AlexandriaException {
		write(fill(new io.intino.goros.unit.box.actions.PostExportNodesAction()).execute());
	}

	private io.intino.goros.unit.box.actions.PostExportNodesAction fill(io.intino.goros.unit.box.actions.PostExportNodesAction action) {
		action.box = this.box;
		action.context = context();
		action.ids = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromQuery("ids"), String.class);
		return action;
	}
