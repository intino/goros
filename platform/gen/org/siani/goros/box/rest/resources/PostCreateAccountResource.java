...
public class PostCreateAccountResource implements Resource {
	...
	public PostCreateAccountResource(GorosBox box, SparkManager manager) {
		this.box = box;
	...
	public void execute() throws Unknown {
		write(fill(new org.siani.goros.box.actions.PostCreateAccountAction()).execute());
	}

	private org.siani.goros.box.actions.PostCreateAccountAction fill(org.siani.goros.box.actions.PostCreateAccountAction action) {
		action.box = this.box;
		action.context = context();
		action.code = manager.fromQuery("code", String.class);
		action.fullname = manager.fromQuery("fullname", String.class);
		action.email = manager.fromQuery("email", String.class);
		action.roles = manager.fromQuery("roles", String.class);
		return action;
	}
