...
public class PostAddDatastoreDimensionComponentResource implements Resource {
	...
	public PostAddDatastoreDimensionComponentResource(UnitBox box, SparkManager manager) {
		this.box = box;
		this.manager = manager;
	}

	public void execute() throws AlexandriaException {
		write(fill(new io.intino.goros.unit.box.actions.PostAddDatastoreDimensionComponentAction()).execute());
	}

	private io.intino.goros.unit.box.actions.PostAddDatastoreDimensionComponentAction fill(io.intino.goros.unit.box.actions.PostAddDatastoreDimensionComponentAction action) {
		action.box = this.box;
		action.context = context();
		action.name = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromQuery("name"), String.class);
		action.dimension = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromQuery("dimension"), String.class);
