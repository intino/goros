...
public class IndexDefinitionTemplate extends Template {
	...
	public RuleSet ruleSet() {
		return new RuleSet().add(
			...
			rule().condition((type("view"))).output(literal("Table(itemClass=\"org.monet.space.kernel.model.NodeItem\", scrollingMark=false, format=bordered) ")).output(mark("definition", "firstUpperCase")).output(mark("name", "firstUpperCase")).output(literal("Table as Selectable(multiple=false)\n    noItemsMessage = \"No hay elementos\"\n    ")).output(expression().output(mark("attribute").multiple("\n"))),
