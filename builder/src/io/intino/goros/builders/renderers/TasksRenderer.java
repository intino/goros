...
public class TasksRenderer extends Renderer {
	...
	protected void writeJava(FrameBuilder builder) {
		...
		writeFrame(file, new io.intino.goros.builders.renderers.templates.java.TasksTemplate().render(builder.toFrame()));
	}

	protected void writeKonos(FrameBuilder builder) {
		File file = new File(konosPackage() + "Tasks.konos");
		writeFrame(file, new io.intino.goros.builders.renderers.templates.konos.TasksTemplate().render(builder.toFrame()));
	}
