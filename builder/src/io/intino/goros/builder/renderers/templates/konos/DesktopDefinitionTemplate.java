...
public class DesktopDefinitionTemplate extends Template {
	...
	public RuleSet ruleSet() {
		return new RuleSet().add(
			...
			rule().condition((type("nodeview"))).output(literal("Block(layout=Vertical Flexible, format=smallAirTop spaced)\n    ")).output(mark("show")),
			rule().condition((type("show"))).output(expression().output(mark("item").multiple("\n"))),
			rule().condition((type("item"))).output(literal("Block(format=doubleAirBottom, visible=false) ")).output(mark("name", "firstLowerCase")).output(literal("\n    Link(title=\"")).output(mark("label")).output(literal("\", format=h5) ")).output(mark("name", "firstLowerCase")).output(literal("Link as Action Addressable(")).output(mark("module", "firstUpperCase")).output(literal("Elements.")).output(mark("name", "firstLowerCase")).output(literal(")\n    Text(value=\"")).output(mark("description")).output(literal("\", format=description)"))
		);
	}
}