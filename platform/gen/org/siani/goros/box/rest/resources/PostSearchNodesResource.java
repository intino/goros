...
public class PostSearchNodesResource implements Resource {

	private GorosBox box;
	private SparkManager<SparkPushService> manager;

	public PostSearchNodesResource(GorosBox box, SparkManager manager) {
		this.box = box;
	...
	public void execute() throws Unknown {
		write(fill(new org.siani.goros.box.actions.PostSearchNodesAction()).execute());
	}

	private org.siani.goros.box.actions.PostSearchNodesAction fill(org.siani.goros.box.actions.PostSearchNodesAction action) {
		action.box = this.box;
		action.context = context();
		action.id = manager.fromQuery("id", String.class);
		action.from = manager.fromQuery("from", String.class);
		action.to = manager.fromQuery("to", String.class);
		return action;
	}
