...
public class FieldSelectDatasource extends TermDatasource {
    ...
    public FieldSelectDatasource(GorosBox box, UISession session, Node node, FieldMultiple<?, ?> field) {
        this(box, session, node, sourceOf(field), (SelectFieldProperty) Dictionary.getInstance().getFieldDefinition(field.getCode()));
    }

    public FieldSelectDatasource(GorosBox box, UISession session, Node node, FieldSelect field) {
        this(box, session, node, field.getSource(), (SelectFieldProperty) field.getDefinition());
    }

    public FieldSelectDatasource(GorosBox box, UISession session, Node node, String source, SelectFieldProperty definition) {
