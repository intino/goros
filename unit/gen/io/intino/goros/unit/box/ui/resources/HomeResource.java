...
import io.intino.goros.unit.box.ui.pages.HomePage;
...
public class HomeResource extends io.intino.alexandria.ui.spark.resources.Resource {
	...
	public HomeResource(UnitBox box, io.intino.alexandria.ui.spark.UISparkManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null) {
				((io.intino.goros.unit.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchHome(client.soul());
				return false;
			}
			...
			((io.intino.goros.unit.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchHome(soul);
