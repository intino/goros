...
public class PostPutLogoResource implements Resource {

	private UnitBox box;
	private SparkManager<SparkPushService> manager;

	public PostPutLogoResource(UnitBox box, SparkManager manager) {
		this.box = box;
		this.manager = manager;
	}

	public void execute() throws AlexandriaException {
		write(fill(new io.intino.goros.unit.box.actions.PostPutLogoAction()).execute());
	}

	private io.intino.goros.unit.box.actions.PostPutLogoAction fill(io.intino.goros.unit.box.actions.PostPutLogoAction action) {
		action.box = this.box;
		action.context = context();
		action.logo = manager.fromForm("logo");
		return action;
	}
