...
public class PostHasPermissionsResource implements Resource {
	...
	public PostHasPermissionsResource(GorosBox box, SparkManager manager) {
		this.box = box;
	...
	public void execute() throws Unknown {
		write(fill(new org.siani.goros.box.actions.PostHasPermissionsAction()).execute());
	}

	private org.siani.goros.box.actions.PostHasPermissionsAction fill(org.siani.goros.box.actions.PostHasPermissionsAction action) {
		action.box = this.box;
		action.context = context();
		action.username = manager.fromQuery("username", String.class);
		return action;
	}
