...
public class PostSaveNodeReferenceResource implements Resource {
	...
	public PostSaveNodeReferenceResource(GorosBox box, SparkManager manager) {
		this.box = box;
	...
	public void execute() throws Unknown {
		write(fill(new org.siani.goros.box.actions.PostSaveNodeReferenceAction()).execute());
	}

	private org.siani.goros.box.actions.PostSaveNodeReferenceAction fill(org.siani.goros.box.actions.PostSaveNodeReferenceAction action) {
		action.box = this.box;
		action.context = context();
		action.id = manager.fromQuery("id", String.class);
		action.code = manager.fromQuery("code", String.class);
		action.attributes = manager.fromQuery("attributes", String.class);
		return action;
	}
