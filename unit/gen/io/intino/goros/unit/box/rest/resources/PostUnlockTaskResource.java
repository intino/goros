...
public class PostUnlockTaskResource implements Resource {
	...
	public PostUnlockTaskResource(UnitBox box, SparkManager manager) {
		this.box = box;
		this.manager = manager;
	}

	public void execute() throws AlexandriaException {
		write(fill(new io.intino.goros.unit.box.actions.PostUnlockTaskAction()).execute());
	}

	private io.intino.goros.unit.box.actions.PostUnlockTaskAction fill(io.intino.goros.unit.box.actions.PostUnlockTaskAction action) {
		action.box = this.box;
		...
		action.stop = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromQuery("stop"), String.class);
		return action;
	}
