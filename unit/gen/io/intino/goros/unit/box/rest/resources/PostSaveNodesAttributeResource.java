...
public class PostSaveNodesAttributeResource implements Resource {
	...
	public PostSaveNodesAttributeResource(UnitBox box, SparkManager manager) {
		this.box = box;
		this.manager = manager;
	}

	public void execute() throws AlexandriaException {
		write(fill(new io.intino.goros.unit.box.actions.PostSaveNodesAttributeAction()).execute());
	}

	private io.intino.goros.unit.box.actions.PostSaveNodesAttributeAction fill(io.intino.goros.unit.box.actions.PostSaveNodesAttributeAction action) {
		action.box = this.box;
		action.context = context();
		action.nodes = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromQuery("nodes"), String.class);
