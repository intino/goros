...
import io.intino.goros.unit.box.ui.pages.RolesPage;
...
public class RolesResource extends io.intino.alexandria.ui.spark.resources.Resource {
	private final UnitBox box;

	public RolesResource(UnitBox box, io.intino.alexandria.ui.spark.UISparkManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		page.session.whenLogin(new Function<String, String>() {
			...
			public String apply(String baseUrl) {
				return RolesResource.this.authenticate(page.session, baseUrl);
			}
		});
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null) {
				((io.intino.goros.unit.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchRoles(client.soul());
				return false;
			}
			...
			((io.intino.goros.unit.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchRoles(soul);
