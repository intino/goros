...
public class PostAddTaskShortcutResource implements Resource {

	private UnitBox box;
	private SparkManager<SparkPushService> manager;

	public PostAddTaskShortcutResource(UnitBox box, SparkManager manager) {
		this.box = box;
		this.manager = manager;
	}

	public void execute() throws AlexandriaException {
		write(fill(new io.intino.goros.unit.box.actions.PostAddTaskShortcutAction()).execute());
	}

	private io.intino.goros.unit.box.actions.PostAddTaskShortcutAction fill(io.intino.goros.unit.box.actions.PostAddTaskShortcutAction action) {
		action.box = this.box;
		...
		action.value = io.intino.alexandria.rest.RequestAdapter.adapt(manager.fromQuery("value"), String.class);
		return action;
	}
