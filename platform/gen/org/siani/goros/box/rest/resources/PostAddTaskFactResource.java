...
public class PostAddTaskFactResource implements Resource {
	...
	public PostAddTaskFactResource(GorosBox box, SparkManager manager) {
		this.box = box;
	...
	public void execute() throws Unknown {
		write(fill(new org.siani.goros.box.actions.PostAddTaskFactAction()).execute());
	}

	private org.siani.goros.box.actions.PostAddTaskFactAction fill(org.siani.goros.box.actions.PostAddTaskFactAction action) {
		action.box = this.box;
		action.context = context();
		action.id = manager.fromQuery("id", String.class);
		action.title = manager.fromQuery("title", String.class);
		action.subtitle = manager.fromQuery("subtitle", String.class);
		action.user = manager.fromQuery("user", String.class);
		action.links = manager.fromQuery("links", String.class);
		return action;
	}
